package lemonflow.ui.operators
{
	...
	import flare.data.DataList;
	import flare.util.Property;
	import flare.util.scale.ScaleBinding;
	 ...
	 */
	public class CircleLayout extends OperatorLayout
	{	
		...
		override public function setup(slots:Vector.<VisualItem>=null):void {
			...
			_rBinding.data = group.dataStructure;
			_aBinding.data = group.dataStructure;
		}
		
		protected override function layout():void
		{			
			var list:DataList = group.dataStructure.group(_group);
			...
			list.visit(function(n:VisualNodeItem):void { n.origin = anchor; });
			...
			if (_aBinding.property) {
				...
				visitor = function(n:VisualNodeItem):void {
					var f:Number = _aBinding.interpolate(_aField.getValue(n));
					n.angle = minAngle(n.angle, startAngle - f*angleWidth);
				};
			} else if (treeLayout) {
				...
				i = 0;
				visitor = function(n:VisualNodeItem):void {
					n.angle = minAngle(n.angle, startAngle - (i/N)*angleWidth);
					i++;
				};
			}
			...
			if (visitor != null) list.visit(visitor);
			if (treeLayout) group.dataStructure.tree.root.radius = 0;
...
//			updateEdgePoints(_t);
		...
		private function setTreeAngles():void
		{
			...
			var root:VisualNodeItem = group.dataStructure.tree.root, p:VisualNodeItem = null;
			var leafCount:int = 0, parentCount:int = 0;
			root.visitTreeDepthFirst(function(n:VisualNodeItem):void {
				if (n.childDegree == 0) {
					if (p != n.parentNode) {
						p = n.parentNode;
						++parentCount;
			...
			root.visitTreeDepthFirst(function(n:VisualNodeItem):void {
				...
				if (n.childDegree == 0) {
					...
					a = n.firstChildNode.angle;
					b = n.lastChildNode.angle - a;
					while (b >  Math.PI) b -= 2*Math.PI;
					while (b < -Math.PI) b += 2*Math.PI;
					a += b / 2;
				}
				n.angle = minAngle(n.angle, a);
			});
		}
		
		private function setTreeRadii():void
		{
			var n:VisualNodeItem;
			var depth:Number = 0, dr:Number = _outer - _inner;
			
			for each (n in group.dataStructure.tree.nodes) {
				if (n.childDegree == 0) {
			...
			for each (n in group.dataStructure.tree.nodes) {
				if (n.childDegree != 0) {
					n.radius = _inner + (n.depth/depth) * dr;
				}
			}
			
			n = group.dataStructure.tree.root;
	        n.x = n.origin.x;
	        n.y = n.origin.y;
		}
