package lemonflow.ui.operators
{
	...
	import flare.display.itemrenderer.ShapeRenderer;
	 ...
	 * ACM CHI 2006.</p>
	 */
	public class CirclePackingLayout extends OperatorLayout
	{
		 ...
		 *  is set to true. */
		public var groupName:String = Data.NODES;
		...
		protected override function layout():void
		{
			...
			var data:Data = group.dataStructure;
			data.nodes.setProperty("shape", Shapes.CIRCLE);
			data.nodes.setProperty("renderer", ShapeRenderer.instance);
			...
			if (treeLayout) {
				...
				if (root.childDegree > 0) {
					...
					root.size = radius / ShapeRenderer.instance.defaultSize;
				}
				shiftTree(root, cn.x, cn.y);
			} else {
				// perform flat layout
				var list:Array = []; data.group(groupName).visit(list.push);
			...
			if (treeLayout) {
				...
				for each (var n:VisualNodeItem in group.dataStructure.group(groupName)) {	
					cn = n.props.chainNode;
					u(n, _anchor.x + scale*cn.x,
...
//			updateEdgePoints(_t);
		...
		private function layoutHelper(n:VisualNodeItem, xc:Number, yc:Number, scale:Number):void
		{
				...
				if (n.expanded) {
					...
					for (i=0; i<n.childDegree; ++i)
						n.getChildNode(i).visitTreeDepthFirst(
							function(n:VisualNodeItem):void {
								u(n, x, y, 0, 0);
							}
						);
				}
		...
		private function u(o:VisualNodeItem, x:Number, y:Number, scale:Number, alpha:Number):void
		{
			o.x = x;
			o.y = y;
			o.size *= scale;
			o.alpha = alpha;
			if (treeLayout && o.parentEdge) 
				o.parentEdge.alpha = alpha;
		}
		
		/** Shift sub-tree coordinate spaces into correct configuration. */
		private function shiftTree(n:VisualNodeItem, nx:Number, ny:Number):void
		{
			for (var i:int=0; i<n.childDegree; ++i) {
				var c:VisualNodeItem = n.getChildNode(i);
		...
		private function packTree(n:VisualNodeItem):Number
		{
		...
		private function getChainNode(n:VisualNodeItem):ChainNode
		{
			var cn:ChainNode, r:Number, size:Number = n.size;
