package lemonflow.ui.operators
{
	 ...
	 * the depth dimension.
	 */
	public class IcicleTreeLayout extends OperatorLayout
	{
...
//			updateEdgePoints(_t);
		...
		private function firstPass(n:VisualNodeItem, d:int):Number
		{
		...
		private function doLayout(n:VisualNodeItem, b1:Number, b2:Number, d:Number, dInc:Number):void
		{
			var pw:Number = n.props.icicleWidth;
			var x:Number = b1, w:Number = b2 - b1;
			
			if (n.childDegree > 0 && !n.expanded) {
				n.visitTreeDepthFirst(function(c:VisualNodeItem):void {
						u(c, b1 + w/2, b1 + w/2, d+dInc, dInc, false);
					});
			} else {
				for (var i:int=0; i<n.childDegree; ++i) {
			...
			u(n, b1, b2, d, dInc, true);
		}
		
		private function u(o:VisualNodeItem, b1:Number, b2:Number, d:Number, dInc:Number, visible:Boolean):void
		{
			if (_vertical) {
				o.x = (b1 + b2) / 2;
				o.y = d + dInc / 2;
			...
			if(o.parentEdge != null)
				o.parentEdge.alpha = visible ? 1 : 0;
		}
